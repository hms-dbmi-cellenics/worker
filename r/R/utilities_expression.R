...
#' @param all_cell_ids vector of cell IDs (0-based)
#' corresponding to rows in expression_values
#'
#' @return sparse matrix with full dimensions (n_full_cells x genes)
#'
expand_matrix_to_cell_ids <- function(expression_values, all_cell_ids) {
  # Convert to triplet format
  trip <- as(expression_values, "TsparseMatrix")

  # Map row indices to their actual cell_ids
  # trip@i is 0-based row position in current matrix,
  # all_cell_ids gives the cell_id for that row
  # cell_ids are 0-based, convert to 1-based for sparseMatrix
  new_i <- all_cell_ids[trip@i + 1] + 1
  new_j <- trip@j + 1
...
#' @param cell_ids vector of cell IDs to get expression values for
#'
#' @return list to send to the UI
#' @export
#'
get_gene_expression <- function(data, genes, cell_ids) {

  expression_values <- get_expression_values(data, genes)
  ordered_gene_names <- ensure_is_list_in_json(colnames(expression_values))

  # get_stats uses the expression values for all cells
  stats <- get_stats(expression_values)
  all_cell_ids <- data@meta.data$cells_id

  if (!is.null(cell_ids)) {
    # subset expression values to requested cell IDs
    cell_idx <- match(cell_ids, all_cell_ids)
    expression_values <- expression_values[cell_idx, , drop = FALSE]

  } else {
    # Expand matrix to full dimensions based on cell_ids
    # This ensures row indices correspond to cell_id
    # positions in the full dataset
    expression_values <- expand_matrix_to_cell_ids(
      expression_values, all_cell_ids
    )
  }

  # Format sparse matrix directly to JSON
  raw_expression <- to_sparse_json(expression_values)
...
#'
#' @inheritParams get_gene_expression
#'
#' @return sparse matrix of expression values (cells x genes)
#' @export
#'
get_expression_values <- function(data, genes) {
  ...
  raw_expression <- Matrix::t(mat)

  # Rename columns to display names
  symbol_idx <- match(colnames(raw_expression), genes$input)
  colnames(raw_expression) <- genes$name[symbol_idx]
...
#'
get_quantile_cap <- function(x, quantile_threshold) {

  # Calculate quantiles for all columns at once
  lims <- sparseMatrixStats::colQuantiles(
    x, probs = quantile_threshold, na.rm = TRUE, drop = TRUE
  )
...
get_stats <- function(expression) {
  ...
  max_vals <- get_quantile_cap(expression, QUANTILE_THRESHOLD)
