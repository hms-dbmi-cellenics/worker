...
class Response:
    ...
    def publish(self):
        ...
        # Remove the temporal file to transfer data between R and python
        # To free up memory
        if self.result.data == config.INTERNAL_RESULTS_PATH_GZ:
            info("Cleaning up temp files generated by work result")
            os.remove(config.INTERNAL_RESULTS_PATH_GZ)
