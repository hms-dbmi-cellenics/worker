...
class CountMatrix:
    ...
    def sync(self):
        ...
        io.Emit(f'ExperimentUpdates-{self.config.EXPERIMENT_ID}', {"info": "downloading seurat object"})
        synced = {
            key: self.download_object(key, last_modified)
            for key, last_modified in objects.items()
        }

        io.Emit(f'ExperimentUpdates-{self.config.EXPERIMENT_ID}', {"info": "checking if R worker is alive"})
